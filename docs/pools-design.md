...
## 10. 压测数据（v0.5.0 · 2026-09-19）

脚本：`benchmarks/pool_bench.py`（每场景 3 轮取最优，timeit 惯例）；
原始数据：`benchmarks/results/pool_bench.json`；复现：`python benchmarks/pool_bench.py`。

环境：Windows 10 · Python 3.10.19 · Intel Comet Lake（12 逻辑核）· 单机。
注：Windows 定时器粒度使 `sleep(0.001)` 实际约 **15.6ms/任务**（2000 任务串行 31.2s
可反推）——场景仍是纯等待型 IO，加速比结论不受影响。

### 10.1 IO 密集（等待型任务 ×2000，串行 31.17s）—— ThreadPool

| workers | 墙钟 (s) | 加速比 | 并行效率 |
|---:|---:|---:|---:|
| 1 | 31.16 | ×1.00 | 100% |
| 2 | 15.59 | ×2.00 | 100% |
| 4 | 7.82 | ×3.99 | 100% |
...
stdlib `ThreadPoolExecutor`（w=8）3.90s vs 本池 3.91s → **包装层端到端开销 ≈ 0**（比值 1.00）。

### 10.2 CPU 密集（纯 Python 计算 103ms/任务 ×24，串行 2.39s）—— ProcessPool

| workers | 墙钟 (s) | 加速比 |
|---:|---:|---:|
| 1 | 2.55 | ×0.94（IPC 开销 6%） |
| 2 | 1.29 | ×1.85 |
| 4 | 0.81 | ×2.95 |
| 8 | 0.57 | **×4.19** |

对照：**ThreadPool w=8 跑同样 CPU 任务 ×1.01**——GIL 佐证，CPU 密集必须用进程池。
4.19x（对 12 逻辑核效率 52%）符合纯 Python 多进程在超线程核上的典型表现。

### 10.3 协程池（1ms 等待 ×5000，串行 await 77.96s）—— AsyncPool

| workers | 墙钟 (s) | 相对串行加速比 |
|---:|---:|---:|
| 8 | 0.54 | **×145** |
| 16 | 0.59 | ×132 |
| 32 | 0.57 | ×137 |
...
对照：无界 `asyncio.gather` 0.10s（5000 任务全并发）。差距来自本池的**受控并发**
（`workers` 令牌，anyio 语义）——是有意提供的背压能力，不是损失。

### 10.4 提交时延（调用方单次 `submit()` 开销，noop 任务 ×20000）

| 实现 | 单次开销 |
|---|---:|
| `lizysdk.pools.ThreadPool` | 56.7 µs |
| stdlib `ThreadPoolExecutor` | 14.7 µs（比值 ×3.9） |

差距来源：ctx 拷贝（`contextvars.copy_context`）、计数/闸门锁、task_id 与钩子查表——
10.1 已证明这些成本在真实工作负载（任务毫秒级以上）中被完全摊薄（端到端与 stdlib 持平）。

### 10.5 特性开销（相对裸配置的每任务额外成本，IO 场景）

| 配置 | 每任务额外成本 |
|---|---:|
| 八钩子全开 | ≈ 噪声（−6.6µs，即 ±10µs 测量噪声内） |
| 关闭 ctx 传播 | ≈ 噪声（−9.3µs） |
| 重试配置（max_retries=1） | +1.6 µs |
| 超时配置（task_timeout=5s） | +2.5 µs |

钩子/ctx/重试配置的常态开销低于本机测量噪声；超时配置因每任务计时线程略高，
量级仍可忽略。

### 结论

- **ThreadPool 近理想线性扩展**（16 worker ×15.9、效率 100%），包装层零损耗
- **ProcessPool 真并行**：CPU 密集 ×4.19（8 worker），且线程池对照组证明 GIL 场景必须选对 kind
- **AsyncPool ×130~145**：受控并发下的协程吞吐
- 调用方 submit 56.7µs 与特性级 µs 开销，在毫秒级以上任务的实际负载中不可见
